...
export function SearchPanel() {
  ...
  /* ── Scroll helpers ─────────────────────────────────────────────────── */
  const getViewport = (ref: React.RefObject<HTMLDivElement | null>) =>
    ref.current?.querySelector('[data-radix-scroll-area-viewport]') as HTMLElement | null

  // Reset scroll to top whenever chapter or book changes
  useEffect(() => {
    const viewport = getViewport(bookScrollRef)
    if (viewport) viewport.scrollTop = 0
    setShowScrollTop(false)
  }, [selectedBookNumber, chapter])

  // Track scroll position to show/hide "Back to top" button
  useEffect(() => {
    const viewport = getViewport(bookScrollRef)
    if (!viewport) return
    const onScroll = () => setShowScrollTop(viewport.scrollTop > 300)
  ...
  return (
    <div
      ...
      className="flex h-full min-h-0 flex-col overflow-hidden rounded-lg border border-border bg-card outline-none"
      onKeyDown={activeTab === "book" ? handleKeyDown : undefined}
      tabIndex={-1}
    >
      {/* STICKY: Tab row + search input — NEVER scrolls */}
      ...
      {activeTab === "book" && (
        <>
          ...
          <div className="relative flex flex-col min-h-0 flex-1">
            <ScrollArea
              key={`book-${selectedBookNumber}-${chapter}`}
              ref={bookScrollRef}
              className="h-full"
            >
            ...
            {showScrollTop && (
              <Button
                variant="secondary"
                size="sm"
                className="absolute bottom-3 right-3 z-50 h-8 shadow-lg border border-border/50 bg-background/90 backdrop-blur-sm hover:bg-background"
                onClick={() => {
                  const viewport = getViewport(bookScrollRef)
                  if (viewport) viewport.scrollTop = 0
                  setShowScrollTop(false)
              ...
              >
                <ArrowUpIcon className="size-3.5 mr-1" />
                Top
              </Button>
            )}
          </div>
      ...
      {activeTab === "context" && (
        <div className="relative flex flex-col min-h-0 flex-1">
          <ScrollArea ref={contextScrollRef} className="h-full">
            <div className="flex flex-col gap-0 p-2">
