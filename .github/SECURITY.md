...
This document describes Lifestone's Content Security Policy and how to report vulnerabilities.

## Content Security Policy

Lifestone sets the webview CSP via `src-tauri/tauri.conf.json`. CSP applies only to the webview (JavaScript, HTML, and CSS loaded inside the Tauri window); it does **not** gate network calls made by the Rust process.
