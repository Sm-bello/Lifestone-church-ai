...
Much of Lifestone's behavior depends on artifacts that aren't checked into the repo: the SQLite Bible database, copyrighted translations, the Qwen3 ONNX embedding model, and precomputed verse embeddings. For anything touching verse detection, Bible search, or rendering, you'll want these in place.
...
Broadcast output via NDI requires the NDI 6 SDK. It's only needed if you're working on `lifestone-broadcast` or the Theme Designer's live output path.
...
| `cargo test -p lifestone-detection` | Tests for one crate |
...
- **Cross-platform work**: Lifestone ships on macOS, Windows, and Linux. If you only tested on one, say so — the "Tested on" checklist is how we know.
