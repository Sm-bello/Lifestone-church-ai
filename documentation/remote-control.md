...
Lifestone provides two remote control protocols for external integration: **OSC** (Open Sound Control) and **HTTP API**. These allow you to control broadcasts, navigate verses, switch themes, and adjust settings from hardware controllers, automation scripts, or custom dashboards.
...
If accessing Lifestone from another device on your network:
...
2. **Add Generic OSC module**:
   - Host: `127.0.0.1` (or your Lifestone computer's IP)
   - Port: `8000`
...
2. Set destination to Lifestone computer IP:8000
...
While Lifestone uses NDI for video output, you can use remote control for automation:
