...
impl Default for VadConfig {
    fn default() -> Self {
        ...
        // TUNED FOR SERMON/CONTINUOUS SPEECH:
        // - Higher thresholds = less sensitive to background noise
        // - Longer min_voice = avoid triggering on coughs/clicks
        // - Longer silence = don't chop mid-sentence pauses
        // - Longer max_utterance = allow full sentences before force-flush
        Self {
            silence_threshold: 0.012,      // ↑ from 0.005 — ignore quiet noise
            frame_threshold: 0.006,        // ↑ from 0.0025
            overall_threshold: 0.08,       // ↑ from 0.05 — need stronger voice signal
            silence_frame_count: 25,       // ↑ from 12 — ~1.6s silence to end (allows breathing pauses)
            min_voice_frames: 8,           // ↑ from 4 — ~0.5s voice to start (ignore clicks)
            max_utterance_frames: 375,     // ↑ from 240 — ~24s max (full sentences)
            pre_buffer_frames: 6,          // ↑ from 4 — ~0.4s pre-buffer (captures speech onset better)
        }
    }
}
