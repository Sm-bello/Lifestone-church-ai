...
//!
//! Usage:
//!   cargo run -p lifestone-detection --features onnx,vector-search --bin precompute -- \
...
fn main() {
    ...
    log::info!("=== Lifestone Verse Embedding Pre-computation ===");
    ...
    let mut embedder = lifestone_detection::OnnxEmbedder::load(
    ...
    log::info!(
        ...
        lifestone_detection::semantic::embedder::TextEmbedder::dimension(&embedder)
    );
    ...
    // Run pre-computation
    lifestone_detection::semantic::precompute::precompute_embeddings(
        &embedder,
        &verses,
