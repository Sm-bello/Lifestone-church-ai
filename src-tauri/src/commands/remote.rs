...
pub async fn get_osc_status(
    ...
    Ok(OscStatus {
        running: runtime.handle.as_ref().is_some_and(lifestone_api::OscHandle::is_active),
...
pub async fn get_http_status(
    ...
    Ok(HttpStatus {
        running: runtime.handle.as_ref().is_some_and(lifestone_api::HttpHandle::is_active),
        port: runtime.bound_port,
    })
}
