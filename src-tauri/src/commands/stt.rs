...
use lifestone_audio::{AudioConfig, AudioFrame};
use lifestone_stt::{DeepgramClient, SttConfig, SttProvider, TranscriptEvent};
...
pub async fn start_transcription(
    ...
    let stt_provider: Box<dyn SttProvider> = match provider_name {
        ...
        "whisper" => {
            ...
            Box::new(lifestone_stt::WhisperProvider::new(
                model_path,
                None,
                n_threads,
            ))
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            'outer: loop {
                ...
                let capture = match lifestone_audio::capture::start(
                    config,
                    audio_tx,
                    device_lost.clone(),
                ) {
                ...
                loop {
                    ...
                    match audio_rx.recv_timeout(Duration::from_millis(100)) {
                        Ok(frame) => {
                            ...
                            if frame_count % 4 == 0 {
                                let level = lifestone_audio::meter::compute_level(&frame.samples);
                                let _ = fan_app.emit(
                                    EVENT_AUDIO_LEVEL,
                                    AudioLevelPayload {
                                        rms: level.rms,
...
fn run_direct_detection(app: &AppHandle, transcript: &str) -> bool {
    use lifestone_detection::{DirectDetector, DetectionMerger};
...
fn check_reading_mode(app: &AppHandle, transcript: &str, direct_found: bool) -> bool {
    use lifestone_detection::ReadingMode;
    ...
    if direct_found {
        let verse_info = {
            let detector_state: State<'_, Mutex<lifestone_detection::DirectDetector>> = app.state();
        ...
        if let Some(change) = chapter_change {
            ...
            if let Some(chapter_verses) = chapter_data {
                if !chapter_verses.is_empty() {
                    ...
                    let advance = lifestone_detection::ReadingAdvance {
