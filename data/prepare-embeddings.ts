/**
 * Unified pipeline: sets up everything needed for Lifestone from scratch.
 *
 *   Phase 1 – Python environment (.venv + all pip deps)
 *   Phase 2 – Download Bible data (pre-built zip + cross-refs)
 *   Phase 3 – Build lifestone.db (SQLite + FTS5)
...
const DB_PATH = join(DATA_DIR, "lifestone.db")
...
async function main() {
  console.log("\n╔══════════════════════════════════════════════╗")
  console.log("║   Lifestone – Full Setup Pipeline                ║")
  console.log("╚══════════════════════════════════════════════╝")
  ...
  if (!shouldSkip("verses JSON", VERSES_JSON)) {
    if (!existsSync(DB_PATH)) {
      console.error(
        "  ❌ lifestone.db not found. Run phases 2-3 first (or remove --force skip)."
      )
      process.exit(1)
    }
